	...
	var page = {
		...
		"PagePaint" : function(){
			var inviter = {
				...
				avatar: url.statice + GetQueryString("avatar")
			};
			...
			$("#mobile").attr("placeholder",("输入手机号，接受" + decodeURI(inviter.name) + "的邀约"));
		},
		"EventSet":function(){
			var c = this;
			$("#accept").on("click",function(){
				...
				$.ajax({
					...
					success:function(data){
						_czc.push(["_trackEvent", "invite", "receive", "v2", 1, "receive"]);
						if(data.ok){
							if(data.code == "success"){
								window.location.href = window.location.origin + "/public/invitev2/success.html";
							}
						}else{
